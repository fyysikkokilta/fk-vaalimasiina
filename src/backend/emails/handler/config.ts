...
const mailTransporter: Transporter = (() => {
  if (process.env.MAILGUN_API_KEY) {
    if (!process.env.MAILGUN_DOMAIN) {
      throw new Error(
        'Invalid email config: MAILGUN_DOMAIN must be set with MAILGUN_API_KEY.'
      )
    }
  ...
  if (process.env.SMTP_HOST) {
    if (!process.env.SMTP_USER || !process.env.SMTP_PASSWORD) {
      throw new Error(
        'Invalid email config: SMTP_USER and SMTP_PASSWORD must be set with SMTP_HOST.'
      )
    }
    return nodemailer.createTransport({
  ...
  console.warn(
    'Neither Mailgun nor SMTP is configured. Falling back to debug mail service.'
  )
  return nodemailer.createTransport({
